...
int main(){
    ...
    for(int i=1;i<=q;i++){
        ...
        if(q == 1){
            ...
            int diff = x - values[s];
            values[s] += diff;
            modification_query(tree, 0, 0, 2*n-1, index1, diff);
            modification_query(tree, 0, 0, 2*n-1, index2, diff);
        }
        else{
            int s;
            cin >> s;
            int index1 = first[s];
            int index2 = last[s];
            cout << sum_query(tree, 0, 0, 2*n-1, index1, index2)/2 << "\n";
        }
    }
